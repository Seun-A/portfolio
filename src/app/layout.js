import "@/styles/globals.scss"
import { Croissant_One, Nunito } from 'next/font/google'
import { ThemeProvider } from "./theme-provider"
import Header from "@/components/navigation/Header"
import Sidebar from "@/components/navigation/Sidebar"
import CopyText from "@/components/CoptText"

const croissant_one = Croissant_One({
  subsets: ['latin'],
  display: 'swap',
  variable: '--font-croissant',
  weight: ['400']
})

const nunito = Nunito({
  subsets: ['latin'],
  display: 'swap',
  variable: '--font-nunito',
})


export const metadata = {
  title: 'Seun-A',
  description: 'Portfolio',
}

export default function RootLayout({ children }) {
 return (
    <html lang="en" className={`${croissant_one.variable} ${nunito.variable}`}>
      <body className="flex flex-col md:flex-row overflow-hidden h-screen">
        <ThemeProvider attribute="class">
          <Sidebar />
          {/* <div className="sticky top-0 z-20">
            <Header />
          </div> */}

          <main className="app grow overflow-scroll px-4 py-2 md:px-8 md:py-8">
            {children}

            <br />
            <CopyText />
          </main>
        </ThemeProvider>

        {/* Animated Circles */}
        { new Array(10).fill().map((e, i) => <div key={i} className={`circle c-${i}`} />) }
      </body>
    </html>
  )
}
